  ...
  ns.createRenderer = function (parser, model) {
    function convertScheduleToCalendar(section) {
      ...
      function updateTooltip(c, e) {
        ...
        if (c.isHoliday) {
          html += '<div class="mp-calendar__tooltip-holiday">' + escapeHtml(labels.holiday_label || '🇰🇷 KR 휴장 (국내 증시 휴장)') + '</div>';
        }
        ...
        c.tooltipData.events.forEach(function (ev) {
          html += '<div class="mp-calendar__tooltip-item is-' + ev.importance + '">' +
            ...
            '<span class="mp-status-chip is-mini is-' + model.getStatusBadgeClass(ev.status) + '">' + escapeHtml(ev.status) + '</span>' +
      ...
      function renderUpcomingList() {
        ...
        visible.forEach(function (e) {
          ...
          if (currentDateStr !== lastDateStr) {
            ...
            dateDivider.innerHTML = '<span class="mp-upcoming__date-text">' + escapeHtml(currentDateStr) + '</span>';
          ...
          var kstStr = (typeof parser.formatKst === 'function') ? parser.formatKst(e.dateTime) : '';
          var timePart = kstStr.split(' ')[1] || '00:00';
          var timeStr = timePart === '00:00' ? '--:--' : timePart;
          timeAxis.innerHTML = '<span class="mp-upcoming__time">' + escapeHtml(timeStr) + '</span><span class="mp-upcoming__dot-line"></span>';
