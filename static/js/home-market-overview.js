(function () {
  ...
  function generateSparkline(data, color) {
    ...
    return (
      '<svg class="mp-sparkline" viewBox="0 0 ' +
      w +
      " " +
      h +
  ...
  function renderTickerGroups(ts) {
    ...
    for (var g = 0; g < GROUPS.length; g++) {
      ...
      for (var t = 0; t < group.tickers.length; t++) {
        ...
        var isPositive =
          !isNeutral && (ticker.invertColor ? changePct < 0 : changePct >= 0);

        var changeColor = isNeutral
          ? "rgb(var(--color-neutral-400))"
          : isPositive
            ? "var(--mp-ticker-up)"
            : "var(--mp-ticker-down)";
        var sparkColor = isNeutral
          ? neutralSparkColor
          : isPositive
        ...
        var valueHtml =
          ...
          ticker.fmt +
          '">' +
          fmtPrice(current, ticker.fmt) +
          "</span>";
        var tickerLabel = ticker.shortName || ticker.name;
        var tickerTitle = ticker.fullName || ticker.name;

        rows +=
          '<div class="mp-ticker-row">' +
          '<span class="mp-ticker-name" title="' +
          escapeHtml(tickerTitle) +
          '">' +
          escapeHtml(tickerLabel) +
          "</span>" +
          valueHtml +
          '<span class="mp-ticker-change" style="color:' +
          changeColor +
          '">' +
          fmtChange(changePct) +
          "</span>" +
      ...
      html +=
        ...
        '<div class="mp-ticker-group__title">' +
        group.title +
        "</div>" +
        rows +
        "</div>";
  ...
  function render(data) {
    ...
    var color =
      REGIME_COLORS[regime && regime.current] ||
      REGIME_COLORS.CAUTIOUS ||
      fallbackColor;
    ...
    document.dispatchEvent(
      new CustomEvent("mp:regime-ready", {
  ...
  function fetchData() {
    ...
    for (var i = 0; i <= lookbackDays; i++) {
      urls.push(
        chartDataPrefix + fmt(new Date(now.getTime() - 86400000 * i)) + ".json",
      );
    }
    tryFetch(urls, 0);
  }
