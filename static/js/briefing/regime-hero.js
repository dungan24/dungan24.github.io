(function() {
  ...
  function getRegimeColor(regime, colorMap, rgbMap) {
    ...
    var color = cm[regime] || defaults.regime[regime] || defaults.regime.CAUTIOUS || '#fbbf24';
    var rgb = rm[regime] || defaults.rgb[regime] || defaults.rgb.CAUTIOUS || '251 191 36';
  ...
  function calcChange(series, key) {
    ...
    for (var i = s.length - 1; i >= 0; i--) {
      if (s[i] != null) {
        if (curr === null) { curr = s[i]; }
        else if (prev === null) { prev = s[i]; break; }
      }
    }
  ...
  function buildMetricHtml(label, value) {
    ...
    return '<div class="mp-quick-metric">' +
      '<span class="mp-qm-label">' + label + '</span>' +
      '<span class="mp-qm-val ' + colorClass + '">' + (v >= 0 ? '+' : '') + value + '%</span>' +
      '</div>';
  }
  ...
  function findArticleHeader() {
    return document.querySelector('header.mt-12') ||
    ...
    hero.innerHTML =
      '<div class="mp-post-hero__top">' +
        '<div class="mp-post-hero__badges">' +
          (regime
            ? '<span class="mp-regime-badge">' + regimeIcon + ' ' + regime + '</span>'
  ...
  function listenChartData(colorMap, rgbMap) {
    function onChartReady(e) {
      ...
      if (!hero.querySelector('.mp-quick-metrics') && data.timeSeries && data.timeSeries.series) {
