(function () {
  ...
  function resolveRegimeColor(text) {
    ...
    for (var i = 0; i < keys.length; i++) {
      if (lower.indexOf(keys[i]) !== -1) {
        ...
        var colors =
          (window.MP_CONFIG && window.MP_CONFIG.colors && window.MP_CONFIG.colors.regime) || {};
  ...
  ns.transformSoWhatSection = function (opinionSubEl) {
    ...
    for (var k = 0; k < allStrongs.length; k++) {
      ...
      var isConfirm =
        stText.indexOf('확인 조건') !== -1 || stText.indexOf('Confirmation Triggers') !== -1;
      var isInvalid =
        stText.indexOf('무효화 기준') !== -1 || stText.indexOf('Invalidation Rules') !== -1;
      ...
      if (isConfirm) {
        ...
        for (var ci = 0; ci < confirmItems.length; ci++) {
          if (!confirmItems[ci].querySelector('.mp-so-what-icon')) {
            ...
            confirmItems[ci].insertBefore(checkIcon, confirmItems[ci].firstChild);
          }
        }
      }
      ...
      if (isInvalid) {
        ...
        for (var ii = 0; ii < invalidItems.length; ii++) {
          if (!invalidItems[ii].querySelector('.mp-so-what-icon')) {
            ...
            invalidItems[ii].insertBefore(crossIcon, invalidItems[ii].firstChild);
          }
        }
      }
    }
