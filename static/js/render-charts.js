(function () {
  ...
  var THEME_RERENDER_DELAY_MS = Number(CHART_CONFIG.theme_rerender_delay_ms || 150);
  ...
  function renderTimeSeries(data) {
    ...
    tooltipStyle.formatter = function (params) {
      var res = '<div style="margin-bottom:8px; font-weight:700; color:' + (dark ? COLORS.cyan : COLORS.primary) + '">' + params[0].name + '</div>';
      params.forEach(function (item) {
        var val = item.value;
        var sign = val > 0 ? '+' : '';
        res += '<div style="display:flex; justify-content:space-between; gap:20px; margin-bottom:4px;">' +
          '<span style="color:' + theme.text + '">' + item.seriesName + '</span>' +
          '<span style="font-weight:700; color:' + (val >= 0 ? theme.success : theme.danger) + '">' + sign + val + '%</span>' +
          '</div>';
      });
      return res;
    };
    ...
    chart.setOption({
      ...
      grid: { top: 30, left: '3%', right: '3%', bottom: 40, containLabel: true },
      xAxis: {
        type: 'category',
        data: dates,
  ...
  function renderCorrelations(data) {
    var chart = createChart('chart-correlation');
    if (!chart || !data.correlation) return;

    var correlations = data.correlation;
    var theme = getTheme();
    var dark = isDarkMode();
    var threshold = 0.5;
    ...
    tooltipStyle.formatter = function (params) {
      ...
      return '<div style="font-weight:700; margin-bottom:4px;">' + p.name + '</div>' +
        '<div style="color:' + getCorrColor(p.value) + '; font-weight:800; font-size:14px;">' +
        sign + p.value.toFixed(2) + '</div>';
    };
  ...
  function renderRegime(data) {
    ...
    var regimeColor = score >= 75 ? COLORS.green :
      score >= 50 ? COLORS.yellow :
        score >= 25 ? COLORS.pink : COLORS.panic;
    ...
    chart.setOption({
      ...
      series: [{
        ...
        pointer: { length: '12%', width: 6, offsetCenter: [0, '-60%'], itemStyle: { color: regimeColor } },
  ...
  function renderSectors(data) {
    ...
    function renderSingle(id, items) {
      ...
      var sorted = items.slice().sort(function (a, b) { return b.week1 - a.week1; });
      var names = sorted.map(function (s) { return s.name; });
      var values = sorted.map(function (s) { return s.week1; });
      ...
      tooltipStyle.formatter = function (params) {
        var p = params[0];
        var sign = p.value > 0 ? '+' : '';
        return '<b>' + p.name + '</b>: ' + sign + p.value.toFixed(2) + '%';
      };
