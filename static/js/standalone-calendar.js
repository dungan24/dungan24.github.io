...
(function () {
  ...
  if (chartDataPrefix.charAt(0) !== '/') chartDataPrefix = '/' + chartDataPrefix;
  ...
  fetchAndMerge().then(function (mergedEvents) {
    ...
    function rebuildGrid(year, month) {
      ...
      cells.forEach(function (c) {
        ...
        cell.addEventListener('click', function () {
          if (window.innerWidth > 768) return;
          if (c.tooltipData && (c.tooltipData.events.length > 0 || c.isHoliday)) {
            showMobileSheet(c);
          }
        });
      ...
      titleEl.childNodes[0].textContent = titleBase + ' \u2014 ' + year + '.' + String(month).padStart(2, '0');
    }
  }
  ...
  function updateTooltip(c, e, sharedTooltip, model) {
    ...
    var html = '<div class="mp-calendar__tooltip-header">' + escapeHtml(c.key) + '</div>';
    if (c.isHoliday) {
      html += '<div class="mp-calendar__tooltip-holiday">' + escapeHtml(labels.holiday_label || '\uD83C\uDDF0\uD83C\uDDF7 KR 휴장') + '</div>';
    }
    ...
    c.tooltipData.events.forEach(function (ev) {
      html += '<div class="mp-calendar__tooltip-item is-' + ev.importance + '">' +
        ...
        '<span class="mp-calendar__tooltip-time">' + escapeHtml(ev.time) + '</span>' +
        '<span class="mp-calendar__tooltip-country">[' + escapeHtml(ev.country) + ']</span>' +
        '</div>' +
        '<div class="mp-calendar__tooltip-name">' + escapeHtml(ev.nameKo || ev.name) + '</div>' +
        '<div class="mp-calendar__tooltip-status">' +
        '<span class="mp-status-chip is-mini is-' + (model ? model.getStatusBadgeClass(ev.status) : 'scheduled') + '">' + escapeHtml(ev.status) + '</span>' +
        '<span class="mp-importance is-mini is-' + ev.importance + '">' + escapeHtml(ev.importance.toUpperCase()) + '</span>' +
        '</div></div>';
    });
    html += '</div>';
  ...
  function showMobileSheet(cellData) {
    ...
    var html = '<div class="mp-mobile-sheet__header">' + escapeHtml(cellData.key) + '</div>';

    if (cellData.isHoliday) {
      html += '<div class="mp-mobile-sheet__holiday">\uD83C\uDDF0\uD83C\uDDF7 KR 휴장 (국내 증시 휴장)</div>';
    }
    ...
    if (cellData.tooltipData && cellData.tooltipData.events.length > 0) {
      ...
      cellData.tooltipData.events.forEach(function (ev) {
        ...
        html += '<div class="mp-mobile-sheet__event is-' + ev.importance + '">' +
          '<div class="mp-mobile-sheet__event-top">' +
          '<span class="mp-mobile-sheet__time">' + escapeHtml(ev.time) + '</span>' +
          '<span class="mp-mobile-sheet__country">[' + escapeHtml(ev.country) + ']</span>' +
          '<span class="mp-importance is-mini is-' + ev.importance + '">' + escapeHtml(ev.importance.toUpperCase()) + '</span>' +
          '</div>' +
          '<div class="mp-mobile-sheet__event-name">' + escapeHtml(ev.nameKo || ev.name) + '</div>' +
          surpriseHtml +
          '</div>';
      });
      html += '</div>';
  ...
  function triggerMobileSheetFromCell(cell) {
    ...
    setTimeout(function () {
      if (!tooltip.classList.contains('is-active') && !cell.classList.contains('is-holiday')) {
        return;
      }
      ...
      items.forEach(function (item) {
        ...
        events.push({
          ...
          country: countryEl ? countryEl.textContent.replace(/[\[\]]/g, '') : '',
  ...
  function addSurpriseIndicators() {
    ...
    metaElements.forEach(function (meta) {
      ...
      Object.keys(eventMap).forEach(function (key) {
        if (key.indexOf(cleanName) >= 0 || cleanName.indexOf(key.replace(/^\[.+?\]\s*/, '')) >= 0) {
          ev = eventMap[key];
        }
      });
