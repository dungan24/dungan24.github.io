...
- [x] **Wave 1**: 컬러 스킴 + 글래스모피즘 카드 (합침)
- [x] **Wave 2**: 배경 장식 + 타이포그래피
- [x] **Wave 3**: 마이크로 애니메이션
- [x] **Wave 4**: 차트 폴리시
- [ ] **Wave 5**: 레이아웃 마무리 + 최종 검증 (최종 시각 검증 남음)

### 실행 로그 (2026-02-11)

- [x] Wave 1~4 코드 반영 완료 (`custom.css`, `render-charts.js`, `custom.html`, `extend-head-uncached.html`, `market-charts.html`, `hero.svg`)
- [x] `hugo -D` 빌드 성공 확인 (경고 1건: Blowfish 모듈 Hugo 버전 호환성 경고)
- [x] 산출물 기반 기능 검증: 차트 라벨/툴팁/zoom, collapsible, TOC scrollspy, search modal, 404, tags
- [ ] 최종 시각 검증(홈/포스트 다크/라이트, 375px 가로스크롤) 및 체크리스트 마감
...
- [x] 모바일 768px: 2열 그리드
- [x] 차트 5종 인터랙션 (tooltip, zoom)
- [x] 접기/펼치기 전체 작동
- [x] TOC 스크롤스파이
- [x] prefers-reduced-motion: 애니메이션+blur 비활성화
- [x] Blowfish 검색 모달 정상 (Metis)
- [x] 404 페이지 정상 (Metis)
- [x] 태그 목록 페이지 정상 (Metis)
- [ ] 커밋 & PR
```
