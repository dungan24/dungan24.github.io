...
확인 사항:
0. renderer.js가 `.mp-calendar` 요소에 `mp-glass-card` 클래스를 실제로 할당하는지 확인: `grep "mp-glass-card" static/js/calendar/renderer.js` (CAL-01 전제 조건)
...
6. mp-glass-card 클래스 할당 확인: `grep "mp-glass-card" static/js/calendar/renderer.js` (CAL-01 전제 조건: renderer.js가 mp-glass-card 클래스를 할당해야 CSS가 적용됨)
  </verify>
  <done>
