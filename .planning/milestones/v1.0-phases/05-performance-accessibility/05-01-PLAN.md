...
- @media prefers-color-scheme 사용하지 않는다 (.dark 클래스만 사용)
  ...
  <verify>
다음 context-aware CSS grep으로 중첩 위반이 0건인지 확인 (셀렉터 블록 + 속성을 함께 검색):

```bash
# .prose table 블록 내부에 backdrop-filter가 없어야 함
rg -A 15 "^\.prose table \{" assets/css/custom.css | rg "backdrop-filter" | rg -v "none" && echo "FAIL: .prose table still has backdrop-filter" || echo "PASS"

# .mp-upcoming__content 블록 내부에 backdrop-filter가 없어야 함
rg -A 15 "\.mp-upcoming__content" assets/css/custom/calendar.css | rg "backdrop-filter" | rg -v "none" && echo "FAIL: .mp-upcoming__content still has backdrop-filter" || echo "PASS"

# .mp-regime-panel 블록 내부에 backdrop-filter가 없어야 함
rg -A 15 "\.mp-regime-panel" assets/css/custom/layout-overrides.css | rg "backdrop-filter" | rg -v "none" && echo "FAIL: .mp-regime-panel still has backdrop-filter" || echo "PASS"

# briefing-sections.css의 .mp-post-hero 블록 내부에 backdrop-filter가 없어야 함
rg -A 15 "\.mp-post-hero" assets/css/custom/briefing-sections.css | rg "backdrop-filter" | rg -v "none" && echo "FAIL: .mp-post-hero in briefing-sections.css still has backdrop-filter" || echo "PASS"
```
