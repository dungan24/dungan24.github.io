---
title: "개장전 브리핑 (Pre-Market)"
date: 2026-02-15T00:00:00+09:00
draft: false
tags: ["pre-market", "브리핑"]
chartData: "/data/chart-data-2026-02-15.json"
---

<!-- OPINION_ZONE_START -->

## 1. 핵심 요약 (30초)

> **S&P +0.05%, 나스닥 -0.22% 혼조세 마감, 크립토 극단적 공포(9)**

- 변동성: VIX 20.6 (-1.06%)
- 금리: 미 10년물 4.06% (-0.05)
- 환율: 원/달러 1,443.3 (+1.79)
- 오늘은: 심리 지표가 낮아 과매도 구간인지 가볍게 체크해두면 좋아요

---

## 2. 오늘의 해석

### 무슨 일이 있었나
미국 증시는 혼조세로 마감했습니다. S&P 500 +0.05%, 나스닥 -0.22%, 다우 +0.10%.
가상자산 시장은 공포탐욕지수 **9(극단적 공포)**으로 바닥권 탐색 중입니다.

### 왜 그런가
S&P 500이 0.05% 상승한 가운데, 나스닥 종합은 0.22% 하락했다.
이는 시장 전반에 긍정적 흐름이 형성되면서 위험자산 선호가 강화되는 모습을 나타내고 있다.
하지만 VIX 지수는 20.6으로 상승했다.
이는 위험자산 선호가 강화되는 상황에서 공포지수가 상승하는 것은 정상적인 상황이다.
또한 이는 시장의 불확실성을 높여주고 있다.
그래서 오늘은: 위험자산 선호가 강화되는 상황에서 주도 테마가 부족하고 종목별 차별화 장세가 나타나면서 시장의 불확실성이 높아지는 요인이 된다.
따라서 오늘의 시장 전반은 위험자산 선호가 강화되는 상황에서 주도 테마가 부족하고 종목별 차별화 장세가 나타나면서 시장의 불확실성이 높아지는 요인이 된다.
기준 시나리오: 변동성 경계가 유지되며 종목별 차별화가 이어집니다.
대안 시나리오: 변동성 진정과 함께 위험자산 선호가 회복됩니다.

공포지수(VIX)는 20.6로 5일 전(17.79) 대비 +15.80% 급변했습니다.

통계적으로 미국 10년물의 최근 변동은 6개월 기준 **하위 9%** 수준으로, 평소보다 드물게 약한 움직임입니다.

주요 자산 간 뚜렷한 연관성이 없는 상태입니다 (최고 상관: S&P 500↔원/달러 +0.13). 각 자산이 독립적으로 움직이고 있어 종목별 대응이 필요합니다.

### 오늘 주목할 것
어제 VIX가 급변(17.79→20.6)했습니다. 오늘 변동성 지속 여부가 핵심입니다.

현재 시장 국면은 **Cautious** 상태로, 방향성 탐색 중. 지표 확인을 위한 관망세 및 변동성 경계

가상자산 공포탐욕지수는 **9(극단적 공포)**. 역사적으로 이 구간에서 단기 반등 확률이 높지만, 추세 전환과는 별개입니다.

### 행동 체크포인트
**관측**: VIX 20.6로 경계 수준
**의미**: 시장 변동성이 점진적으로 상승 중
**시사점**: 심리 지표가 낮아 과매도 구간인지 가볍게 체크해두면 좋아요

- **크립토 극단적 공포(9)** — 시장 심리 바닥권 가능성
  → 역사적으로 단기 반등 확률 높으나, 추세 전환과는 별개

<!-- OPINION_ZONE_END -->

---

<!-- FACT_ZONE_START -->

## 핵심 수치  ⚠ (일부 지연 데이터)

| 항목 | 종가 | 등락 | 등락률 | 흐름 |
|------|------|------|--------|------|
| S&P 500 | 6,836.17 | +3.41 | +0.05% | - |
| 나스닥 | 22,546.67 | -50.48 | -0.22% | - |
| 나스닥 100 | 24,732.73 | +45.12 | +0.18% | - |
| 다우존스 | 49,500.93 | +48.95 | +0.10% | - |
| VIX (공포지수) | 20.6 | -0.22 | -1.06% | - |
| 미국 10년물 국채 | 4.06% | -0.05 | - | - ⚡ 하위 9% |
| 달러인덱스 (DXY) | 96.88 | -0.03 | -0.03% | - |

### 가상자산 

| 항목 | 가격 | 등락 | 등락률 | 흐름 |
|------|------|------|--------|------|
| 비트코인 (BTC) | 69,603 | +1,371.18 | +1.97% | - |
| 이더리움 (ETH) | 2,081.65 | +65.78 | +3.16% | - |
| 공포탐욕지수 | 9 (극단적 공포) | - | - | - |

### 환율  ⚠ (일부 지연 데이터)

| 항목 | 현재가 | 등락 | 등락률 | 흐름 |
|------|--------|------|--------|------|
| 원/달러 (USDKRW) | 1,443.3 | +1.79 | +0.12% | - |

### 지수 선물

> ↗ 본장 +0.05% 마감 → 선물 +0.21% 반등 중 (Basis: +0.21%)

| 항목 | 현재가 | 등락 | 등락률 |
|------|--------|------|--------|
| S&P 500 선물 | 6,850.5 | -0.5 | -0.01% |
| 나스닥 100 선물 | 24,803.25 | +35.25 | +0.14% |

### 원자재

| 항목 | 현재가 | 등락 | 등락률 |
|------|--------|------|--------|
| 국제 금 (Gold) | 5,046.3 | +97.9 | +1.98% |
| WTI 유가 (Oil) | 62.89 | +0.05 | +0.08% |

{{< market-charts >}}

---

## 주요 뉴스  ⚠ (일부 지연 데이터)

(주요 뉴스 없음)

---

## 오늘의 일정  ⚠ (일부 지연 데이터)

(예정된 이벤트 없음)

---

## 시장 분위기(국면)

> 🟡 **Cautious** — 방향성 탐색 중. 지표 확인을 위한 관망세 및 변동성 경계

| 근거 | 값 | 판단 |
|------|-----|------|
| VIX | 20.6 | ⚠ 경계 |
| SPX 20일 | +0.6% | ⚠ 횡보 |
| US10Y 방향 | -9bp (5일) | ✅ 완화 기대 |
| 크립토 심리 | 9 | ⚠ 극단적 공포 (contrarian 반등 시그널) |

## 시장 동조화(연관성 높은 지수들의 패턴 매칭)

현재 주요 자산 간 뚜렷한 연관성 없음 (독립적 움직임)

## 섹터 상대강도

### 미국 섹터

| 섹터 | 1주 초과수익 | 1개월 초과수익 |
|------|--------------|----------------|
| 에너지 (XLE) | +3.17% | +16.86% |
| 헬스케어 (XLV) | +2.71% | +2.65% |
| 반도체 (SOXX) | +2.38% | +12.46% |
| 산업 (XLI) | +2.12% | +9.91% |
| 소비재 (XLY) | +0.68% | -2.47% |
| 테크 (XLK) | -0.80% | -2.54% |
| 금융 (XLF) | -2.40% | -7.03% |

### 한국 섹터 (최근 흐름)

| 섹터 | 1주 초과수익 | 1개월 초과수익 |
|------|--------------|----------------|
| 은행 | +5.85% | +7.22% |
| 자동차 | -0.51% | +4.70% |
| IT | -1.79% | -0.78% |
| 에너지 | -2.18% | -4.43% |
| 바이오 | -3.79% | -23.78% |
| 반도체 | -4.30% | +3.50% |
| 2차전지 | -5.07% | -4.37% |


## 최근 흐름 (5일)

| 지표 | 5일전 | 현재 | 변동 | 방향 |
|------|-------|------|------|------|
| SPX | 6,941.81 | 6,836.17 | -1.52% | 소폭 하락 |
| BTC | 70,120 | 69,603 | -0.74% | 소폭 하락 |
| VIX | 17.79 | 20.6 | +15.80% | 급격 불안 |
| USDKRW | 1,458.54원 | 1,443.3원 | -1.04% | 소폭 하락 |

## 이벤트 임팩트

주요 이벤트 대기 중 (발표 후 반영)

<!-- FACT_ZONE_END -->

---

## 용어 설명

- **DXY**: 달러 가치 지수. 오르면 달러가 강해진다는 뜻
- **선물**: 미래 특정 시점에 사고팔기로 약속한 계약. 장 시작 전 방향을 가늠하는 데 사용
- **10년물**: 미국 10년 만기 국채. 주택담보대출 금리의 기준이 되는 지표
- **VIX**: 시장 불안 지수. 숫자가 클수록 투자자들이 불안해하고 있다는 뜻
- **공포탐욕지수**: 0~100 스케일. 0에 가까울수록 시장이 극도로 두려워하는 상태


---

## 출처

- Yahoo Finance (api) | https://finance.yahoo.com
- CoinGecko (api) | https://api.coingecko.com/api/v3/simple/price
- Alternative.me (api) | https://api.alternative.me/fng/
- 키움증권 REST API (api) | https://api.kiwoom.com
- Static Economic Calendar (file) | file://static-calendar.json
- Yahoo Finance Historical (api) | https://finance.yahoo.com

---

<div class="mp-briefing-meta">
  <span>생성 시각: 2026-02-15 00:17:08 KST</span>
  <span>브리핑 슬롯: pre-market (개장전)</span>
  <span>데이터 기준 시각: 2026-02-15 00:17:08 KST</span>
</div>
